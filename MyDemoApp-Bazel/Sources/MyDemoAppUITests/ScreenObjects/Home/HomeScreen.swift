...
class HomeScreen : BaseScreen {
    ...
    private lazy var twitterText = app.staticTexts[AppConstants.TWITTER]
    private lazy var facebookText = app.staticTexts[AppConstants.FACEBOOK]
    private lazy var linkedinText = app.staticTexts[AppConstants.LINKEDIN]
    ...
    func tapOnSocialMediaButton(_ platform: String) {
        let button: XCUIElement
        // Identify the button based on the platform name
        switch platform.lowercased() {
        case AppConstants.TWITTER:
            button = twitter
        case AppConstants.FACEBOOK:
            button = facebook
        case AppConstants.LINKEDIN:
            button = linkedin
        default:
            XCTFail("Unknown social media platform: \(platform)")
            return
        }
        // Ensure the button exists and tap it
        guard button.exists else {
            XCTFail("\(platform.capitalized) button not found.")
            return
        }
        button.tap()
        TestLogger.shared.log("\(AppConstants.HOME_SCREEN): \(LogMessages.TAPPED_SOCIAL_MEDIA_BUTTON)")
    }
        
        /// Method to verify redirection based on the platform
    func verifyRedirection(for platform: String, timeout: TimeInterval = 5.0) -> Bool {
        // Check for specific elements that indicate redirection
        let expectedElement: XCUIElement
        // Determine the expected redirection element for each platform
        switch platform.lowercased() {
        case AppConstants.TWITTER:
            expectedElement = twitterText // Checks for element in the Redirected Screen
        case AppConstants.FACEBOOK:
            expectedElement = facebookText // Checks for element in the Redirected Screen
        case AppConstants.LINKEDIN:
            expectedElement = linkedinText // Checks for element in the Redirected Screen
        default:
            XCTFail("Social media platform for redirection verification: \(platform)")
            return false
        }
        // Wait for the expected element to appear within the timeout period
        return expectedElement.waitForExistence(timeout: timeout)
    }
}
