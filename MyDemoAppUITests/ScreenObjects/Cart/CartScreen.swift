...
class CartScreen : BaseScreen{
    ...
    private lazy var removeFromCartButton = app.buttons[ElementIdentifiers.REMOVE_ITEM]
    ...
    private lazy var noItemsDescriptionLbl = app.staticTexts[ElementIdentifiers.NO_ITEMS_LABEL]
    private lazy var itemCountLabel = app.staticTexts[ElementIdentifiers.ITEM_COUNT_LABEL]
    ...
    func tapOnRemoveFromCartButton() {
        Helpers.tapOnFirstMatch(in: app, matching: .button, label: ElementIdentifiers.REMOVE_ITEM)
        TestLogger.shared.log("\(AppConstants.CART_SCREEN): \(LogMessages.TAPPED_PROCEED_TO_CHECKOUT)")
    }
    ...
    /// Checks if the "No Items Description" text is visible.
    ///
    /// This function returns `true` if the `noItemDescLbl` UI element is visible on the screen.
    func checkForNoItemsDescText() -> Bool {
        return Helpers.isUIElementVisible(noItemsDescriptionLbl)
    }
